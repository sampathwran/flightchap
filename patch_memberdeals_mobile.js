...
content = content.replace(
  ...
  '<div className="flex flex-row overflow-x-auto lg:grid lg:grid-cols-3 gap-6 pb-6 snap-x snap-mandatory hide-scrollbar">'
);
content = content.replace(
  '<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">',
  '<div className="flex flex-row overflow-x-auto lg:grid lg:grid-cols-3 gap-6 pb-6 snap-x snap-mandatory hide-scrollbar">'
...
// Loading skeleton
content = content.replace(
  /className="h-80 bg-slate-100 rounded-xl animate-pulse"/g,
  'className="w-[85vw] sm:w-[45vw] lg:w-auto shrink-0 h-80 bg-slate-100 rounded-xl animate-pulse"'
);

fs.writeFileSync(file, content, 'utf8');
console.log('Fixed MemberDeals mobile slider');
